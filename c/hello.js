...
async function compileC(code) {
  ...
  try {
    ...
    await comp.fileSystem.writeFile("/working/main.c", code);
    outEl.textContent = "Compiling...\n";
    const result = await comp.run("em++ -x c -std=c18 -sNO_FILESYSTEM=0 -sENVIRONMENT=worker -sUSE_WEBGL2=0 -sSTRICT=1 -fexceptions -sEXIT_RUNTIME=1 -O2 main.c -o main.js -sSINGLE_FILE=1");
    if (result.returncode === 0) {
      ...
      currentRunner = new Worker("../cpp/workerJS.js");
      const output = [];
...
function loadStreak() {
  const raw = localStorage.getItem('c_streak');
  lessonsInRow = raw ? (parseInt(raw, 10) || 0) : 0;
  updateStreakUI();
}

function saveStreak() {
  localStorage.setItem('c_streak', String(lessonsInRow));
}
...
async function setupLogic() {
  ...
  let lastLesson = localStorage.getItem('c_current_lesson');
  ...
  function submitCheck() {
    ...
    if (passed) {
      ...
      localStorage.setItem('c_current_lesson', lessonFileFromUrl);
