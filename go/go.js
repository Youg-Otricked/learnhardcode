...
let suite;
function loadStreak() {
    const raw = localStorage.getItem('go_streak');
    lessonsInRow = raw ? (parseInt(raw, 10) || 0) : 0;
    updateStreakUI();
}

function saveStreak() {
    localStorage.setItem('go_streak', String(lessonsInRow));
}
...
async function loadYaegi() {
    const go = new Go();
    const result = await WebAssembly.instantiateStreaming(fetch("yaegi-wasm/yaegi.wasm"), go.importObject);
    go.run(result.instance);
}
let yaegiReady = false;
loadYaegi().then(() => { yaegiReady = true; });
async function runGo(code) {
    if (!yaegiReady) {
        outEl.textContent = 'Go interpreter still loading...';
        return;
    }
    lastRunOutput = '';
    outEl.className = '';
    try {
        const result = window.yaegi.eval(code);
        console.log("result:", result);
        if (result.success) {
            lastRunOutput = result.output || '';
            outEl.textContent = lastRunOutput;
        } else {
            lastRunOutput = 'Runtime Error: ' + result.error;
            outEl.textContent = lastRunOutput;
...
function markLessonCompleted(lessonId, xpEarned) {
  ...
  localStorage.setItem('go_completed_lessons', JSON.stringify(completed));
}
...
function submitCheck() {
    ...
    if (passed) {
        ...
        localStorage.setItem('go_current_lesson', lessonFileFromUrl);
...
async function setupLogic() {
    ...
    let lastLesson = localStorage.getItem('go_current_lesson');
...
document.addEventListener('DOMContentLoaded', () => {
    require(['vs/editor/editor.main'], function() {
        ...
        console.log('Go Monaco Editor initialized');
        setupLogic();
