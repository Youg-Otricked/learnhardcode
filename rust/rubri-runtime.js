...
class Interpreter {
    constructor(miri, wasi, fds, stdin, stdout, stderr) {
        this.miri = miri;
        this.wasi = wasi;
        this.stdin = stdin;
        this.stdout = stderr;
    ...
    async run(code, printLast = false) {
        this.stdin.clear();
        this.stdout.clear();
        this.stderr.clear();
...
async function load_external_file(path) {
    return new File(await cached_or_fetch(path).then(b => b.blob()).then(b => b.arrayBuffer()));
}
async function cached_or_fetch(path) {
    // Downloads or caches the file from `path`
    // Files of more than 10MB aren't cached by fetch, so it must be done manually
    const base = "./wasm-rustc";
    path = base + path;
    try {
        caches;
    }
