...
export default class WASI {
    ...
    constructor(args, env, fds, options = {}) {
        ...
        this.wasiImport = {
            ...
            clock_res_get(id, res_ptr) {
                ...
                switch (id) {
                    case wasi.CLOCKID_MONOTONIC: {
                        ...
                        resolutionValue = 5000n; // 5 microseconds
                        break;
                    }
                    case wasi.CLOCKID_REALTIME: {
                        resolutionValue = 1000000n; // 1 millisecond?
                        break;
                    }
                    default:
            ...
            fd_prestat_dir_name(fd, path_ptr, 
            // eslint-disable-next-line @typescript-eslint/no-unused-vars
            path_len) {
            ...
            path_rename(fd, old_path_ptr, old_path_len, new_fd, new_path_ptr, new_path_len) {
                ...
                if (self.fds[fd] != undefined && self.fds[new_fd] != undefined) {
                    ...
                    if (ret != wasi.ERRNO_SUCCESS) {
                        if (self.fds[fd].path_link(old_path, inode_obj, true) !=
                            wasi.ERRNO_SUCCESS) {
