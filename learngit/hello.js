...
function loadStreak() {
  const raw = localStorage.getItem('git_streak');
  lessonsInRow = raw ? (parseInt(raw, 10) || 0) : 0;
  updateStreakUI();
}

function saveStreak() {
  localStorage.setItem('git_streak', String(lessonsInRow));
}
...
async function loadLesson(lessonFile) {
  const path = 'learngit/' + lessonFile;
...
function getCompletedLessons() {
  const stored = localStorage.getItem('git_completed_lessons');
...
function markLessonCompleted(lessonId, xpEarned) {
  ...
  localStorage.setItem('git_completed_lessons', JSON.stringify(completed));
}
async function setupLogic() {
  ...
  let lastLesson = localStorage.getItem('git_current_lesson');
  ...
  function submitCheck() {
    ...
    if (passed) {
      ...
      localStorage.setItem('git_current_lesson', lessonFileFromUrl);
