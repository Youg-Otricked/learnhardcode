import {}
const chapters: Record<string, any> = await (await fetch("./chapters.json")).json();
const completedLessons = JSON.parse(localStorage.getItem("cpp_completed_lessons") || '[]');
console.log(chapters);
const completedSet = new Set(Object.keys(completedLessons));
const chaptersElem = document.getElementById("chapters") as HTMLDetailsElement;
...
for (const chapter of chapters["chapters"].sort((a: any, b: any) => a.number - b.number)) {
    const btn = document.createElement("button");
    btn.style.cssText = "background: none; padding: 0; border: none; cursor: pointer; margin: 0 auto; font-size: 19px;";
    const br = document.createElement("br");
    const allCompleted = chapter.lessons.every((l: any) => completedSet.has(l));
