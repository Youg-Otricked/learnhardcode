...
function loadStreak() {
  const raw = localStorage.getItem('linux_streak');
  lessonsInRow = raw ? (parseInt(raw, 10) || 0) : 0;
  updateStreakUI();
}

function saveStreak() {
  localStorage.setItem('linux_streak', String(lessonsInRow));
}
...
function getCompletedLessons() {
  const stored = localStorage.getItem('linux_completed_lessons');
...
function markLessonCompleted(lessonId, xpEarned) {
  ...
  localStorage.setItem('linux_completed_lessons', JSON.stringify(completed));
}
async function setupLogic() {
  ...
  let lastLesson = localStorage.getItem('linux_current_lesson');
  ...
  function submitCheck() {
    ...
    if (passed) {
      ...
      localStorage.setItem('linux_current_lesson', lessonFileFromUrl);
